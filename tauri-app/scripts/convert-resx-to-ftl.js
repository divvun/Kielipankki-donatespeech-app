...
function convertToFluent(key, value) {
  ...
  let fluentValue = value.replace(/\{(\d+)\}/g, (match, num) => `{ $param${num} }`);
...
async function convertResxToFtl(resxPath, outputPath) {
  console.log(`Converting ${path.basename(resxPath)} -> ${path.basename(outputPath)}`);
...
async function main() {
  ...
  // Read all .resx files
  const files = fs.readdirSync(RESX_DIR)
