...
namespace Recorder.Maui.Platforms.Android
{
    public class AudioRecorder : IAudioRecorder
    {
        ...
        public string Prepare()
        {
            ...
            Debug.WriteLine($"Prepared FLAC recording: {fileName}");
            return recordingId;
        }

        /**
         * Start the recorder & encoder process and kick off the recording loop
         * in a new thread (@see RecordingTask()).
         **/
        public void Start()
        {
            if (currentState != State.Prepared)
            {
                throw new RecordingException($"Invalid state {currentState}");
            }
            ...
            Debug.WriteLine($"Started FLAC recording: {audioFile.FileName}");
        }
        ...
        public AudioFile Stop()
        {
            ...
            {
                throw new RecordingException($"Recording failed: {e}");
            }
            finally
            {
        ...
        {
            if (addFinishedRecordingHeaders && encoderOutput != null && checksum != null)
            {
            ...
            Debug.WriteLine($"Finalized FLAC recording: {audioFile.FileName}, Duration: {audioFile.Duration}s, Samples: {sampleCount}");
        ...
        private void RecordingTask(CancellationToken token)
        {
            ...
            {
                Debug.WriteLine($"Recording task error: {e.Message}");
                throw;
            }
            finally
            {
